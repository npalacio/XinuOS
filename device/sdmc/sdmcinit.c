...
{
    volatile struct sdmc_csreg *csrptr;	/* addr of SD controller's CSR	*/
    uint32 pciinfo;			/* PCI info to read config	*/
	
    /* Search for the SD memory card device on the PCI bus */
    pciinfo = find_pci_device(INTEL_QUARK_SDIO_PCI_DID,
	INTEL_QUARK_SDIO_PCI_VID, 0);
    ...
    if(pci_read_config_dword(pciinfo, 0x10,
	(uint32 *)&devptr->dvcsr) == SYSERR) {
    ...
    csrptr->nrm_int_status_en = SDMC_CRD_INS_STAT_EN
	| SDMC_CRD_RMV_STAT_EN  | SDMC_CRD_INT_STAT_EN;
    csrptr->nrm_int_sig_en = SDMC_CRD_INS_SIG_EN
	| SDMC_CRD_RMV_SIG_EN  | SDMC_CRD_INT_SIG_EN;
