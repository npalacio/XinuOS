...
shellcmd xsh_ipaddr(int nargs, char *args[]) {
	...
	uint32	temp;			/* temporary address to display	*/
	int32	i;			/* loop counter */
	int32	shown;			/* did we show a value?		*/
	...
	if (nargs == 2) {
		...
		if (NetData.ipvalid == FALSE) {
			fprintf(stderr, "DHCP request unsuccessful.\n");
			return 1;
		}
	}
	...
	/* IP lease time in seconds */

	printf("   %-15s  %d secs\n", "Lease time:", NetData.leasetime);
	...
	/* RFC 868 time servers in dotted decimal & hex */

	shown = 0;
	for(i=0;i<NTSADDR;i++) {
	    temp = NetData.tsaddr[i];
	    sprintf(str, "%d.%d.%d.%d",
		(temp>>24)&0xff, (temp>>16)&0xff,
		(temp>>8)&0xff, temp&0xff);
	    if (!shown) {
		printf("   %-15s  %-15s  0x%08x\n",
		"Time server(s):", str, temp);
		shown = 1;
	    } else {
		printf("   %-15s  %-15s  0x%08x\n",
		"", str, temp);
	...
	/* RFC 1035 domain name server in dotted decimal & hex */
