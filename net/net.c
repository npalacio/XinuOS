...
void	net_init (void)
{
	...
	/* Create a process to handle UECHO device output buffers */

	resume(create(ueproc, UESTK, UEPRIO, "ueproc", 0, NULL));

	/* Create a process to  handle UECHO connections */

	resume(create(ueconn, UESTK, UEPRIO, "ueconn", 0, NULL));
}
